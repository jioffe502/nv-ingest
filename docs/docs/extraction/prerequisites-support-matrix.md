...
- `nimOperator.answer_llm` deploys the generic answer-generation NIM and auto-wires `/v1/answer`. It defaults to Nemotron 3.5 Lightning and enables its `nemotron_v3` reasoning parser. Override its image, model ID, resources, profile, and environment to run Omni or another OpenAI-compatible NIM in that slot. This opt-in does not populate `serviceConfig.agentic`. Refer to [Self-hosted Helm Nemotron 3.5 Lightning](workflow-agentic-retrieval.md#self-hosted-helm-lightning) for agentic configuration.
...
⁶ The default Helm `answer_llm` Nemotron 3.5 Lightning NIMService (`nvcr.io/nim/nvidia/nemotron-3.5-lightning-30b-a3b:2.0.9-variant`) requests one GPU, sets `NIM_TENSOR_PARALLEL_SIZE=1`, and enables the `nemotron_v3` reasoning parser and `qwen3_coder` tool-call parser. The NIM Operator selects a compatible profile because `modelProfile` is empty. NVIDIA lists TP1 minimums of 66 GB for BF16, 32 GB for W4A16, and 30 GB for NVFP4 on Blackwell. NVIDIA lists the NIM as verified on the table's RTX PRO 6000, B200, H100, A100 80GB, and L40S SKUs. For TP1 on A100 80GB and L40S, select W4A16. H100 BF16 TP1 requires `--max-num-seqs 512` in `NIM_PASSTHROUGH_ARGS`, in addition to the reasoning and tool-call arguments. Higher context length or concurrency needs additional memory headroom. H200 NVL, A100 40GB, and RTX PRO 4500 meet one or more published TP1 profile floors but are not listed as verified in the linked matrix. A10G does not meet a TP1 floor and requires a multi-GPU W4A16 profile plus matching Helm overrides. The chart NIMCache PVC is 250Gi. Refer to the [Nemotron 3.5 Lightning NIM support matrix](https://docs.nvidia.com/nim/large-language-models/latest/reference/support-matrix.html#nemotron-35-lightning-30b-a3b).
