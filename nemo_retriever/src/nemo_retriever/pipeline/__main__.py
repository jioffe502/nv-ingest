...
def run(
    ...
    try:
        ...
        if run_mode == "service":
            ...
            split_config = (
                _split_config_for_input_type(input_type, text_chunk_params.model_dump()) if enable_text_chunk else None
            )
        ...
        if quiet:
            ...
            with _quiet_capture():
                if run_mode == "service":
                    ...
                    assert ingest_plan is not None
                    raw_result = execute_ingest_plan(ingest_plan, **local_execute_kwargs).result
        else:
            if run_mode == "service":
                raw_result = ingestor.ingest()
            else:
