...
def handle_lancedb(
    ...
    Accepts *rows* as a ``pd.DataFrame`` or ``list[dict]`` (e.g. from
    ``take_all()``).  Converts to canonical VDB records, writes to
    LanceDB, and creates search indices.
    """
    import lancedb
    ...
    table = db.create_table(table_name, data=records, schema=schema, mode=mode)
    try:
        create_lancedb_index(table, cfg=cfg)
    except RuntimeError:
        logger.warning(
